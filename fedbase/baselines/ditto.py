...
def run(dataset, batch_size, num_nodes, model, objective, optimizer, global_rounds, local_steps, reg_lam, device = torch.device('cuda' if torch.cuda.is_available() else 'cpu'), **split):
    ...
    for i in range(global_rounds):
        ...
        for j in range(num_nodes):
            nodes[j].local_update_ditto(local_steps, device, server, reg_lam)
